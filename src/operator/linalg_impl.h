...
#pragma message("USE_CBLAS IS SET")
#endif
#if MSHADOW_USE_MKL == 1
#pragma message("USE_MKL IS SET")
#endif
...
#define LINALG_GPU_GEMM_AXIS_OLD(fname, DType) \
template<> inline \
void linalg_gemm_axis<gpu, DType>(const Tensor<gpu, 3, DType>& A, const Tensor<gpu, 3, DType>& B, \
  ...
  for (index_t i = 0; i < A.size(1); ++i) { \
    CUBLAS_CALL(cublas##fname(Stream<gpu>::GetBlasHandle(s), \
                              ...
                              B.dptr_+i*B.stride_, B.size(1)*B.stride_,\
                              A.dptr_+i*A.stride_, A.size(1)*A.stride_, &beta,\
                              C.dptr_+i*C.stride_, C.size(1)*C.stride_)) \
  } \
}
//LINALG_GPU_GEMM_AXIS(Sgemm, float)
//LINALG_GPU_GEMM_AXIS(Dgemm, double)
...
#define LINALG_GPU_BATCH_GEMM_AXIS(fname, DType) \
  template<> inline \
  void linalg_batch_gemm<gpu, DType>(const Tensor<gpu, 4, DType>& A, const Tensor<gpu, 4, DType>& B, \
